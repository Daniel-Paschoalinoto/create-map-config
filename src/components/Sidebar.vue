<template>
    ...
    <ul class="sidebar-menu">
      <li v-for="(item, index) in menuItems" :key="index">
        <template v-if="item.name === 'CLEAR ALL'">
          <button class="clear-all-button" @click="openPopup">
            <component :is="item.icon" class="icon" />
            <span>{{ item.name }}</span>
          </button>
        </template>
        <template v-else>
          <a href="javascript:void(0)" @click="setActiveIndex(index)" :class="{ active: activeIndex === index }">
            <component :is="item.icon" class="icon" />
            <span>{{ item.name }}</span>
          </a>
        </template>
  ...
  <main>
    ...
    <div v-if="activeIndex === 12">
      <TotalAnunciosSiteForm :totalAnunciosSite="total_anuncios_site"
        @updateTotalAnunciosSite="updateTotalAnunciosSite" />
    </div>
  </main>
...
.popup-overlay {
  ...
  align-items: center;
  z-index: 9999;
  /* Colocando o pop-up no topo de todos os outros elementos */
}
...
.popup-content {
  ...
  text-align: center;
  width: 300px;
  /* Tornando o pop-up mais largo */
}

.popup-buttons {
  display: flex;
  justify-content: space-evenly;
  /* Para colocar os botões lado a lado */
  margin-top: 20px;
}
